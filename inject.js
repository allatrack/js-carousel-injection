(function() {
    ...
    function MGInjector(rootId, mainDOM) {
                                ...
                                <div class="image-with-text"> \
                                    ...
                                    <div class="text-elements"> \
                                        <div class="text-on-hover"> \
                                            <div class="mctitle"><a class="mgline-link-' + this.rootId + '" href="{%=encodeURI(o[i][10].l)%}" data-hash="{%#o[i][9]%}" target="_blank" rel="nofollow">{%=o[i][3]%}</a></div> \
                                            <div class="fake"> \
                                                <div class="mcdomain"> \
                                                    <a class="mgline-link-' + this.rootId + '" target="_blank" href="{%=encodeURI(o[i][10].dl)%}" data-hash="{%#o[i][9]%}" rel="nofollow">{%=o[i][0]%}</a> \
                                                </div> \
                                            </div> \
                                            <div class="mgtobottom"> \
                                                <div class="mcdomain"><a class="mgline-link-' + this.rootId + '" href="{%=encodeURI(o[i][10].l)%}" data-hash="{%#o[i][9]%}" target="_blank" rel="nofollow">{%=o[i][0]%}</a></div> \
                                            </div> \
                                        </div> \
                                    </div> \
                                </div> \
        ...
        this.appendJS = function(src, includeCheck) {
            if (includeCheck === undefined) {
                includeCheck = true;
            }
        ...
        this.run = function() {
            ...
            dataScript.onload = function() {
                ...
                carouselScript.onload = function() {
                    ...
                    addEvent(parent.window, "resize", function() {
                        mGCarousel.mgboxWidth = that.mainDOM.getElementById("MarketGidComposite" + that.rootId).clientWidth - 2; // Borders
                        mGCarousel.calculateHeight();
                        mGCarousel.movingSide = 'none';
                        mGCarousel.currentCarouselShift = 0;
                        mGCarousel.transitionalMoving = true;
                        that.mainDOM.getElementById("mgslider-" + that.rootId).style.left = mGCarousel.currentCarouselShift + 'px';
                        mGCarousel.decorateSlider();
                    });
